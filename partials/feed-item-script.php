<script>
    ...
    window.addEventListener('load', function(){
        ...
        document.querySelectorAll('.like-btn').forEach(item=>{
            item.addEventListener('click', ()=>{ //adiciona o evento de click.
                let id = item.closest('.feed-item').getAttribute('data-id'); // --> pega o id da postagem.
                let count = parseInt(item.innerText); // --> pega a quantidade de likes.
                if(item.classList.contains('on') === false) { // verifica se deu like ou deslike.
